...
These instructions are meant for Ubuntu 18.04 or 20.04, or a recent version of macOS (we tested Mojave and Catalina).
...
```
git clone --recursive git@gitlab.vision.in.tum.de:visnav_ws20/<username>/visnav.git
```
...
We recommend to go for the latest release (currently 4.13), but it should be at least 4.8.
...
> ```
> sudo apt install qtcreator-4.13 clang-format
> ```
> You can start this version of QtCreator from the terminal with the command `qtcreator-4.13`.
...
>
> ```
> wget https://download.qt.io/official_releases/qtcreator/4.13/4.13.2/qt-creator-opensource-linux-x86_64-4.13.2.run
> chmod +x qt-creator-opensource-linux-x86_64-4.13.2.run
> ./qt-creator-opensource-linux-x86_64-4.13.2.run
> ```
> On Ubuntu 18.04, the cmake version doesn't satisfy the minimum requirement for QtCreator 4.13. You can install version 4.12 instead. To this end, replace the first line:
> ```
> wget https://download.qt.io/official_releases/qtcreator/4.12/4.12.4/qt-creator-opensource-linux-x86_64-4.12.4.run
> ```
> Alternatively, Ubuntu 18.04 users can install a recent release of cmake via pip (install pip with `sudo apt install python3-pip` if missing):
> ```
